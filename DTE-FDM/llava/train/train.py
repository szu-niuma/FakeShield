...
def train(attn_implementation=None):
    ...
    if model_args.vision_tower is not None:
        if "mpt" in model_args.model_name_or_path:
            ...
            model = LlavaLlamaForCausalLM.from_pretrained(
                ...
                attn_implementation=attn_implementation,  # 使用 Flash Attention 2.0 算法，这是一种优化的注意力算法，可以显著提高速度并减少内存使用
    ...
    """
    梯度检查点:
        常规训练：保存所有中间激活值用于反向传播，消耗大量显存
        梯度检查点：在前向传播时只保存特定检查点的激活值，反向传播时根据需要重新计算其他激活值
    """
