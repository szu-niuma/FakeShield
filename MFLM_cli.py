...
from MFLM.tools.utils import DEFAULT_IM_END_TOKEN, DEFAULT_IM_START_TOKEN, DEFAULT_IMAGE_TOKEN, IMAGE_TOKEN_INDEX
...
def prepare_model_for_inference(model, args):
    ...
    print("\033[92m" + "---- Initialized Global Image Encoder (vision tower) from: {} ----".format(args.vision_tower) + "\033[0m")
...
def inference(input_str, all_inputs, follow_up, generate):
    ...
    # 大模型的vision_tower
    global_enc_image = global_enc_processor.preprocess(image_np, return_tensors="pt")["pixel_values"][0].unsqueeze(0).cuda()
    ...
    grounding_enc_image = grounding_enc_processor(torch.from_numpy(image).permute(2, 0, 1).contiguous()).unsqueeze(0).cuda()
...
if __name__ == "__main__":
    ...
    output_path: Path = Path(args.MFLM_output) / args.target_type
    output_path.mkdir(parents=True, exist_ok=True)  # 创建完整路径，包括所有父目录
    ...
    json_data = read_json(args.DTE_FDM_output + f"{args.target_type}.json")
    total_len = len(json_data)
    cur = 0
    for item in json_data:
        input_image, input_text = item["image"], item["outputs"]
