...
from DTE_FDM.llava.constants import IMAGE_TOKEN_INDEX, DEFAULT_IMAGE_TOKEN, DEFAULT_IM_START_TOKEN, DEFAULT_IM_END_TOKEN
from DTE_FDM.llava.conversation import conv_templates, SeparatorStyle
from DTE_FDM.llava.model.builder import load_pretrained_model
from DTE_FDM.llava.utils import disable_torch_init
from DTE_FDM.llava.mm_utils import process_images, tokenizer_image_token, get_model_name_from_path
...
def DTE_FDM_cli(args):
    ...
    json_path = Path("/home/yuyangxin/data/dataset/MagicBrush/record.json")
    ...
    for item in data.values():
        real_image_path = item[args.target_type]
        real_text = image_process(tokenizer, model, image_processor, context_len, DTG, model_name, real_image_path, args)
        outputs.append({"image": real_image_path, "outputs": real_text})
        cur += 1
        print(f"当前进度: {cur} / {total_len}")

    save_path = args.output_path + f"{args.target_type}.json"
